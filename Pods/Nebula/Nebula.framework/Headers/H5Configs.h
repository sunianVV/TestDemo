...
@property(nonatomic, readonly) BOOL shouldForceCheckJsApiPermission;
...
// schem降级开关
@property(nonatomic, assign, readonly) BOOL  shouldDegradeWKToPage;
// 自定义类（是否忽略 内嵌webview域名白名单管控）
@property(nonatomic, readonly) NSString *customAllowDomainPermissionClsName;

// 自定义类（是否忽略 服务器域名白名单管控）
@property(nonatomic, readonly) NSString *customValidDomainPermissionClsName;

// 是否hook前端系getLocation，转发到Jsapi实现。默认为 YES
@property(nonatomic, assign, readonly) BOOL  shouldHookGetlocation;
// 是否在webview下拉时显示网页来源。默认为 NO，即不显示
@property(nonatomic, assign, readonly) BOOL  shouldShowWebViewDomain;
